...
flowchart TD
    ...
    E --> F[Currency: SOL/USDC<br/>Amount in lamports or token units<br/>Recipient Address<br/>Network]
...
2. **Dynamic Parameter Calculation**:
   - **Uncertainty Factor (U)**: Calculated as `(1.0 - Certainty) / 0.5` for AI certainty in range [0.5, 1.0]
     - When AI is 100% certain (1.0): U = 0.0 (no uncertainty)
     - When AI is 75% certain (0.75): U = 0.5 (moderate uncertainty)
     - When AI is 50% certain (0.5): U = 1.0 (maximum uncertainty)
     - Note: Certainty values below 0.5 are treated as 0.5 (complete uncertainty)
